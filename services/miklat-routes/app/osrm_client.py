"""Тонкий клиент к OSRM HTTP API (route service, профиль foot, N waypoints)."""
...
def get_route(coordinates: list[tuple[float, float]]) -> dict:
    """
    coordinates — упорядоченный список (lon, lat), минимум 2 точки.
    steps=true — нужны legs с разбивкой distance/duration по перегонам между
    точками (в отличие от miklat-walking-routes, где важен только итог).
    """
